...
use App\Models\User;
use App\Models\VerifikasiInternalPartner;
...
class VerifikasiInternalPartnerController extends Controller
{
    ...
    public function index()
    {
        ...
        $all_super_user_counter = MatriksApprovalVerifikasiPartner::all()->filter(function ($user) {
        ...
        $matriks_category = MatriksApprovalVerifikasiPartner::all()->groupBy(['kategori', 'departemen_code', 'divisi_id']);

        $partnerApprovalAll = VerifikasiInternalPartner::with('Proyek')?->whereIn('unit_kerja', $unit_kerjas)->whereIn('divisi_id', $collectDivisiMatriksUser)?->whereIn('departemen_id', $collectDepartemenMatriksUser)?->orderByDesc('updated_at')->get();
        ...
        return view('23_VerifikasiInternalPartner', [
    ...
    public function ProsesRequestApproval(Request $request, Proyek $proyek)
    {
        try {
            ...
            if (empty($request->get("nip"))) {
                return response()->json([
                    "Success" => false,
                    "Message" => "Mohon isi KAM terlebih dahulu"
                ]);
            }
            ...
            if (!File::isDirectory(public_path('file-nota-rekomendasi-2/file-verifikasi-internal-partner/'))) {
                File::makeDirectory(public_path('file-nota-rekomendasi-2/file-verifikasi-internal-partner/'));
            }

            $namaFile = date("dmYHis_") . $proyek->kode_proyek . "_Dokumen_Verifikasi_Internal_Penentuan_KSO.pdf";

            $pdf->save(public_path('file-nota-rekomendasi-2/file-verifikasi-internal-partner/' . $namaFile));
            ...
            if (empty($isExistVerifikasi)) {
                $newApproval = new VerifikasiInternalPartner();
                ...
                if ($newApproval->save()) {
                    foreach ($nomorTarget as $target) {
                        $url = $request->schemeAndHttpHost() . "?nip=" . $target->Pegawai->nip . "&redirectTo=/verifikasi-internal-partner?open=kt_modal_view_proyek_rekomendasi_" . $proyek->kode_proyek;
                        $message = "Yth Bapak/Ibu " . $target->Pegawai->nama_pegawai . "\nDengan ini mengajukan varifikasi internal partner untuk proyek " . $proyek->nama_proyek . " untuk permohonan pengajuan assessment partner.\nSilahkan tekan link di bawah ini untuk proses selanjutnya.\n\n$url\n\nTerimakasih 🙏🏻";
                        $sendEmailUser = sendNotifEmail($target->Pegawai, "Permohonan Pengajuan Verifikasi Internal Partner", nl2br($message), $this->isnomorTargetActive);
                        if (!$sendEmailUser) {
                            return redirect()->back();
                        }
                    }
                ...
                if ($isExistVerifikasi->save()) {
                    foreach ($nomorTarget as $target) {
                        $url = $request->schemeAndHttpHost() . "?nip=" . $target->Pegawai->nip . "&redirectTo=/verifikasi-internal-partner?open=kt_modal_view_proyek_rekomendasi_" . $proyek->kode_proyek;
                        $message = "Yth Bapak/Ibu " . $target->Pegawai->nama_pegawai . "\nDengan ini mengajukan varifikasi internal partner untuk proyek " . $proyek->Proyek->nama_proyek . " untuk permohonan pengajuan assessment partner.\nSilahkan tekan link di bawah ini untuk proses selanjutnya.\n\n$url\n\nTerimakasih 🙏🏻";
                        $sendEmailUser = sendNotifEmail($target->Pegawai, "Permohonan Pengajuan Verifikasi Internal Partner", nl2br($message), $this->isnomorTargetActive);
                        if (!$sendEmailUser) {
                            return redirect()->back();
                        }
            ...
            // Alert::success("Pengajuan Berhasil", "Proses Pengajuan Verifikasi Internal Partner Berhasil Diajukan");
            // return redirect()->back();
            return response()->json([
                'Success' => true,
                'Message' => "Proses Pengajuan Verifikasi Internal Partner Berhasil Diajukan"
            ]);
        } catch (\Throwable $th) {
            // return response()->json([
            //     'Success' => false,
     ...
     */
    public function ProsesPengajuanApproval(Request $request, VerifikasiInternalPartner $proyek)
    {
        $data = $request->all();
        $is_paralel = true;
        ...
        try {
            ...
            if (isset($data["is_approved"])) {
                ...
                if ($is_paralel) {
                    ...
                    if (empty($getNomorMatriks)) {
                        return redirect()->back();
                    }

                    foreach ($getNomorMatriks as $target) {
                        $url = $request->schemeAndHttpHost() . "?nip=" . $target->Pegawai->nip . "&redirectTo=/verifikasi-internal-partner?open=kt_modal_pengajuan_verifikasi_" . $proyek->kode_proyek;
                        ...
                        $sendEmailUser = sendNotifEmail($target->Pegawai, "Permohonan Pemberian Rekomendasi Verifikasi Internal Partner", nl2br($message), $this->isnomorTargetActive);
                        if (!$sendEmailUser) {
                            return redirect()->back();
                        }
                    }
                    ...
                    if ($is_done) {
                        ...
                        $getNomorMatriks = $this->getNomorMatriksApproval($proyek->divisi_id, $proyek->departemen_id, "Pengajuan", 2);
                        if (empty($getNomorMatriks)) {
                            return redirect()->back();
                        }
                ...
                foreach ($getNomorMatriks as $target) {
                    $url = $request->schemeAndHttpHost() . "?nip=" . $target->Pegawai->nip . "&redirectTo=/nota-rekomendasi-2?open=kt_modal_pengajuan_verifikasi_" . $proyek->kode_proyek;
                    ...
                    $sendEmailUser = sendNotifEmail($target->Pegawai, "Pemberitahuan Revisi Pengajuan Verifikasi Internal Partner", nl2br($message), $this->isnomorTargetActive);
                    if (!$sendEmailUser) {
                        return redirect()->back();
                    }
                }
     ...
     */
    public function ProsesRekomendasiApproval(Request $request, VerifikasiInternalPartner $proyek)
    {
        $data = $request->all();
        $is_paralel = false;
        ...
        try {
            ...
            if (isset($data["is_approved"])) {
                ...
                if ($is_paralel) {
                    ...
                    foreach ($getNomorMatriks as $target) {
                        $url = $request->schemeAndHttpHost() . "?nip=" . $target->Pegawai->nip . "&redirectTo=/verifikasi-internal-partner?open=kt_modal_persetujuan_verifikasi_" . $proyek->kode_proyek;
                        ...
                        $sendEmailUser = sendNotifEmail($target->Pegawai, "Permohonan Persetujuan Verifikasi Internal Partner", nl2br($message), $this->isnomorTargetActive);
                        if (!$sendEmailUser) {
                            return redirect()->back();
                        }
                    }
                    ...
                    if ($is_done) {
                        ...
                        $getNomorMatriks = $this->getNomorMatriksApproval($proyek->divisi_id, $proyek->departemen_id, "Rekomendasi", 2);
                        if (empty($getNomorMatriks)) {
                            return redirect()->back();
                        }
                ...
                foreach ($getNomorMatriks as $target) {
                    $url = $request->schemeAndHttpHost() . "?nip=" . $target->Pegawai->nip . "&redirectTo=/nota-rekomendasi-2?open=kt_modal_persetujuan_verifikasi_" . $proyek->kode_proyek;
                    $message = "Yth Bapak/Ibu " . $target->Pegawai->nama_pegawai . "\nDengan ini menyampaikan permohonan revisi verifikasi internal partner untuk proyek " . $proyek->Proyek->nama_proyek . " untuk permohonan pengajuan assessment partner.\nSilahkan tekan link di bawah ini untuk proses selanjutnya.\n\n$url\n\nTerimakasih 🙏🏻";
                    $sendEmailUser = sendNotifEmail($target->Pegawai, "Pemberitahuan Revisi Persetujuan Verifikasi Internal Partner", nl2br($message), $this->isnomorTargetActive);
                    if (!$sendEmailUser) {
                        return redirect()->back();
                    }
                }
     ...
     */
    public function ProsesPersetujuanApproval(Request $request, VerifikasiInternalPartner $proyek)
    {
        $data = $request->all();
        $is_paralel = false;
            ...
            if (isset($data["is_approved"])) {
                ...
                if ($is_paralel) {
                    ...
                    if ($is_done) {
                        ...
                        $getNomorMatriks = $this->getNomorMatriksApproval($proyek->divisi_id, $proyek->departemen_id, "Persetujuan", 2);
                        if (empty($getNomorMatriks)) {
                            return redirect()->back();
                        }
                        foreach ($getNomorMatriks as $target) {
    ...
    {
        try {
            $ProyekNotaQrSelected = VerifikasiInternalPartner::where('kode_proyek', $kode_proyek)->first();
    ...
    {
        try {
            $matriksSelected = MatriksApprovalVerifikasiPartner::where('nama_pegawai', $user->nip)
    ...
    {
        try {
            return MatriksApprovalVerifikasiPartner::where("divisi_id", "=", $divisi_id)->where('departemen_code', $departemen_code)->where("kategori", "=", $kategori)->count() == $approved_data->count();
        } catch (\Throwable $th) {
            throw $th;
        }
    }
     ...
     * @return \App\Models\MatriksApprovalVerifikasiPartner
     */
    private function getNomorMatriksApproval($divisi_id, $departemen_code, $kategori, $urutan = null)
    {
        try {
            $matriksSelected = MatriksApprovalVerifikasiPartner::where("divisi_id", "=", $divisi_id)->where('departemen_code', $departemen_code)->where("kategori", "=", $kategori)->get();
    ...
    {
        try {
            $verifikasiPartner = $proyek->VerifikasiInternalPartner;

            $verifikasiKSO = collect(json_decode($proyek->alasan_kso));

            if ($verifikasiKSO->isEmpty()) {
                Alert::error("Error", "Mohon isi Verifikasi Internal KSO terlebih dahulu!");
                return redirect()->back();
            }
            ...
            $approvedPengajuan->each(function ($item) use ($request, $proyek, $pathQRPengajuan) {
                ...
                $qrPathPengajuan = public_path("/template-ttd/verif-internal-partner/" . $qrNamePengajuan);
                $urlPengajuan = $request->schemeAndHttpHost() . "?nip=" . $item->nip . "&redirectTo=/verifikasi-internal-partner/" . $proyek->kode_proyek . "/" . $item->nip . "/view-qr?kategori=pengajuan";
            ...
            $approvedRekomendasi->each(function ($item) use ($request, $proyek, $pathQRRekomendasi) {
                ...
                $qrPathRekomendasi = public_path("/template-ttd/verif-internal-partner/" . $qrNameRekomendasi);
                $urlRekomendasi = $request->schemeAndHttpHost() . "?nip=" . $item->nip . "&redirectTo=/verifikasi-internal-partner/" . $proyek->kode_proyek . "/" . $item->nip . "/view-qr?kategori=rekomendasi";
            ...
            $approvedPersetujuan->each(function ($item) use ($request, $proyek, $pathQRPersetujuan) {
                ...
                $qrPathPersetujuan = public_path("/template-ttd/verif-internal-partner/" . $qrNamePersetujuan);
                $urlPersetujuan = $request->schemeAndHttpHost() . "?nip=" . $item->nip . "&redirectTo=/verifikasi-internal-partner/" . $proyek->kode_proyek . "/" . $item->nip . "/view-qr?kategori=persetujuan";
            ...
            $pdf = Pdf::loadView('GenerateFile.view', ["verifikasiKSO" => $verifikasiKSO, 'jenisProyek' => $proyek->jenis_proyek, "pathQRPengajuan" => $pathQRPengajuan, "pathQRRekomendasi" => $pathQRRekomendasi, "pathQRPersetujuan" => $pathQRPersetujuan]);
            $pdf->setPaper('A4', 'landscape');
            if (!File::isDirectory(public_path('file-nota-rekomendasi-2/file-verifikasi-internal-partner/'))) {
                File::makeDirectory(public_path('file-nota-rekomendasi-2/file-verifikasi-internal-partner/'));
            }

...
{
    private function sendDataSAP($profit_center, $periode)
    {
        // $claims_all = PerubahanKontrak::all();
        $claims_all = ContractApproval::whereIn("jenis_perubahan", ["VO", "Klaim"])->whereIn("stage", [1, 2, 4, 5])->where("profit_center", "=", $profit_center)->where('periode', '=', $periode)->get();
    ...
    public function index(Request $request){
        ...
        $approvals = $contract_approval->map(function ($approval) {
            ...
            $nama_proyek = $approval->first()->ProyekPISNew->proyek_shortname;
            //Kategori VO
            ...
            return [
                ...
                'profit_center' => $approval->first()->ProyekPISNew->profit_center ?? '',
                'periode' => $month,
    ...
    public function lockApprovalRev(Request $request)
    {
        ...
        $approval = ContractApproval::where('profit_center', '=', $data['profit_center'])->where('periode', '=', $data["periode"])->get();
        ...
        ])->where('profit_center', '=', $data['profit_center'])->get();
        if (!empty($approval->toArray()) || $approval->isNotEmpty()) {
            ...
            $data_approval = $claims->map(function ($claim) use ($periode) {
                ...
                $claim['profit_center'] = $claim->profit_center;
                $claim->makeHidden(['Proyek', 'id', 'ProyekPISNew']); //Untuk menghilangkan relasi agar tidak masuk ke array
                return $claim;
            });
    ...
    public function setUnlock(Request $request){
        ...
        $approval = ContractApproval::where("profit_center", "=", $data["profit_center"])->where("periode", $month)->first();
    ...
    public function setApprove(Request $request, $profit_center){
        $data = $request->all();
        ...
        $approval = ContractApproval::where("profit_center", "=", $profit_center)->where("periode", $periode);
        ...
        if($data["approve"] == 't'){
            ...
            if ($update) {
                $get_response = $this->sendDataSAP($profit_center, $periode);

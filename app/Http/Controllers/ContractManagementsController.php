...
use App\Models\DokumenAanwitjzingContract;
...
class ContractManagementsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (Gate::any(['super-admin', 'admin-ccm'])) {
            ...
            //     return self::stdClassToModel($data, Proyek::class);
            // });
            $tahun_proyeks = Proyek::get()->groupBy("tahun_perolehan")->keys()->sortDesc();
            ...
            if (!empty($proyekSelected)) {
                if (!empty($filterBulan) && $filterTahun == date('Y')) {
                    ...
                    if ($filterTahun < 2023 && !empty($filterBulan)) {
                        ...
                        $proyeks_all = Proyek::join(
                            ...
                            "=",
                            "proyeks.kode_proyek"
                        )->where("tahun_perolehan", "<=", $filterTahun)->where("bulan_pelaksanaan", "<=", 12)->whereIn("unit_kerja", $unit_kerja_get)->whereIn("jenis_proyek", $jenis_proyek_get)->whereIn('contract_managements.profit_center', $proyekSelected)->get();
                        // dd("test");
                    }
                }
                ...
                if (!empty($filterBulan) && $filterTahun == 2023) {
                    ...
                    if ($filterTahun < date("Y") && !empty($filterBulan)) {
                        $proyeks_all = Proyek::join("contract_managements", "contract_managements.project_id", "=", "proyeks.kode_proyek")->where("tahun_perolehan", "<=", $filterTahun)->where("bulan_pelaksanaan", "<=", $filterBulan)->whereIn("unit_kerja", $unit_kerja_get)->whereIn("jenis_proyek", $jenis_proyek_get)->get();
                    } elseif ($filterTahun < date("Y") && empty($filterBulan)) {
                        $proyeks_all = Proyek::join("contract_managements", "contract_managements.project_id", "=", "proyeks.kode_proyek")->where("tahun_perolehan", "<=", $filterTahun)->where("bulan_pelaksanaan", "<=", 12)->whereIn("unit_kerja", $unit_kerja_get)->whereIn("jenis_proyek", $jenis_proyek_get)->get();
                    } else {
                        $proyeks_all = Proyek::join("contract_managements", "contract_managements.project_id", "=", "proyeks.kode_proyek")->where("tahun_perolehan", "=", $filterTahun)->where("bulan_pelaksanaan", "<=", 12)->whereIn("unit_kerja", $unit_kerja_get)->whereIn("jenis_proyek", $jenis_proyek_get)->get();
                        // dd("test");
    ...
    public function viewContract($profit_center)
    {
        ...
        if (empty($contract->id_contract)) {
            $tesss = ContractManagements::where('profit_center', $contract->profit_center)
                ->update(['id_contract' => Str::uuid()->toString()]);
        }

        if (empty($contract->contract_proceed) && !empty($contract->profit_center)) {
            $proyekPISNew = ProyekPISNew::where('profit_center', $contract->profit_center)->first();
            $tes = ContractManagements::where(function ($query) use ($contract) {
                $query->where('id_contract', "=", $contract->id_contract)
                    ->orWhere('profit_center', '=', $contract->profit_center);
            })
                ->update([
                    'contract_proceed' => "Belum Selesai",
                    'contract_in' => $proyekPISNew->start_date,
                    'contract_out' => $proyekPISNew->finish_date,
                    'value' => (int)$proyekPISNew->contract_value_idr,
                    'no_contract' => $proyekPISNew->contract_no,
                    'number_spk' => $proyekPISNew->spk_intern_no,
                ]);
        }
    ...
    public function siteInstruction(Request $request, SiteInstruction $siteInstruction)
    {
        ...
        if ($siteInstruction->save() && $uploadFinal->save()) {
            ...
            $file->move(public_path('contract-managements/dokumen-site-instruction'), $id_document);
    ...
    public function deleteSiteInstruction(SiteInstruction $id)
    {
        ...
        if ($file->delete()) {
            File::delete(public_path("contract-managements/dokumen-site-instruction/$nama_file"));
            return (object)[
                'success' => true,
    ...
    public function technicalForm(Request $request, TechnicalForm $technicalForm)
    {
        ...
        if ($technicalForm->save() && $uploadFinal->save()) {
            // moveFileTemp($file, explode(".", $id_document)[0]);
            $file->move(public_path('contract-managements/dokumen-technical-form'), $id_document);
    ...
    public function deleteTechnicalForm(TechnicalForm $id)
    {
        ...
        $nama_file = $file->id_document;
        if ($file->delete()) {
            File::delete(public_path("contract-managements/dokumen-technical-form/$nama_file"));
            return (object)[
                'success' => true,
                'message' => "Dokumen berhasil dihapus",
            ];
    ...
    public function technicalQuery(Request $request, TechnicalQuery $technicalQuery)
    {
        ...
        if ($technicalQuery->save() && $uploadFinal->save()) {
            $file->move(public_path('contract-managements/dokumen-technical-query'), $id_document);
            // moveFileTemp($file, explode(".", $id_document)[0]);
    ...
    public function deleteTechnicalQuery(TechnicalQuery $id)
    {
        ...
        $nama_file = $file->id_document;
        if ($file->delete()) {
            File::delete(public_path("contract-managements/dokumen-technical-query/$nama_file"));
            return (object)[
                'success' => true,
                'message' => "Dokumen berhasil dihapus",
            ];
    ...
    public function fieldChange(Request $request, FieldChange $fieldChange)
    {
        ...
        if ($fieldChange->save() && $uploadFinal->save()) {
            $file->move(public_path('contract-managements/dokumen-field-design-change'), $id_document);
            // moveFileTemp($file, explode(".", $id_document)[0]);
    ...
    public function deleteFieldChange(FieldChange $id)
    {
        ...
        $nama_file = $file->id_document;
        if ($file->delete()) {
            File::delete(public_path("contract-managements/dokumen-field-design-change/$nama_file"));
            return (object)[
                'success' => true,
                'message' => "Dokumen berhasil dihapus",
            ];
    ...
    public function changeNotice(Request $request, ContractChangeNotice $changeNotice)
    {
        ...
        if ($changeNotice->save() && $uploadFinal->save()) {
            $file->move(public_path('contract-managements/dokumen-change-notice'), $id_document);
            // moveFileTemp($file, explode(".", $id_document)[0]);
    ...
    public function deleteChangeNotice(ContractChangeNotice $id)
    {
        ...
        $nama_file = $file->id_document;
        if ($file->delete()) {
            File::delete(public_path("contract-managements/dokumen-change-notice/$nama_file"));
            return (object)[
                'success' => true,
                'message' => "Dokumen berhasil dihapus",
            ];
    ...
    public function changeOrder(Request $request, ContractChangeOrder $changeOrder)
    {
        ...
        if ($changeOrder->save() && $uploadFinal->save()) {
            $file->move(public_path('contract-managements/dokumen-change-order'), $id_document);
            // moveFileTemp($file, explode(".", $id_document)[0]);
    ...
    public function deleteChangeOrder(ContractChangeOrder $id)
    {
        ...
        $nama_file = $file->id_document;
        if ($file->delete()) {
            File::delete(public_path("contract-managements/dokumen-change-order/$nama_file"));
            return (object)[
                'success' => true,
                'message' => "Dokumen berhasil dihapus",
            ];
    ...
    public function changeProposal(Request $request, ContractChangeProposal $changeProposal)
    {
        ...
        if ($changeProposal->save() && $uploadFinal->save()) {
            $file->move(public_path('contract-managements/dokumen-change-proposal'), $id_document);
            // moveFileTemp($file, explode(".", $id_document)[0]);
    ...
    public function deleteChangeProposal(ContractChangeProposal $id)
    {
        ...
        $nama_file = $file->id_document;
        if ($file->delete()) {
            File::delete(public_path("contract-managements/dokumen-change-proposal/$nama_file"));
            return (object)[
                'success' => true,
                'message' => "Dokumen berhasil dihapus",
            ];
    ...
    public function documentBastContractUpload(Request $request)
    {
        ...
        if($dokumen->save()){
            if (isset($data['dokumen-bast-1'])) {
                $data['dokumen-bast-1']->move(public_path('contract-managements/dokumen-bast'), $id_document);
            }
    ...
    public function documentBastContractEdit(Request $request, $id_bast) 
    {
        ...
        if ($dokumen->save()) {
            if (!empty($data['dokumen-bast-1'])) {
                File::delete(public_path("contract-managements/dokumen-bast/$old_file"));
                $data['dokumen-bast-1']->move(public_path("contract-managements/dokumen-bast"), $id_document);
            }
    ...
    public function deleteBast($id_document){
        ...
        File::delete(public_path("contract-managements/dokumen-bast/" . $id_document));
        // if (str_contains($documentBast->nama_dokumen, '.pdf')) {
        //     File::delete(public_path("contract-managements/dokumen-bast/" . $id_document . '.pdf'));
        // } else {
        //     File::delete(public_path("contract-managements/dokumen-bast/" . $id_document . '.docx'));
        // }
    ...
    public function baDefectContractUpload(Request $request)
    {
        ...
        if (count($data["ba-defect"]) > 1) {
            ...
            foreach ($data["ba-defect"] as $dokumen_ba_defect) {
                $id_document = $faker->uuid3() . $dokumen_ba_defect->getClientOriginalExtension();
                array_push($list_id_document_pendukung, $id_document);
                // moveFileTemp($dokumen_ba_defect, $id_document);
                $dokumen_ba_defect->move(public_path("/contract-managements/dokumen-ba-defect"), $id_document);
            }
            $contract->list_dokumen_ba_defect = join(",", $list_id_document_pendukung);
        } else {
            // $id_document = $faker->uuid3();
            $id_document = $faker->uuid3() . $data["ba-defect"][0]->getClientOriginalExtension();
            // moveFileTemp($data["ba-defect"][0], $id_document);
            $data["ba-defect"][0]->move(public_path("/contract-managements/dokumen-ba-defect"), $id_document);
    ...
    public function dokumenPendukungContractUpload(Request $request)
    {
        ...
        if (!empty($contract->dokumen_kontrak_dan_addendum)) {
            ...
            foreach ($list_dokumen_kontrak_dan_addendum as $dokumen) {
                $get_dokumen = File::get(public_path("/contract-managements/dokumen-pendukung-contract/$dokumen"));
                if (!empty($get_dokumen)) {
                    File::delete(public_path("/contract-managements/dokumen-pendukung-contract/$dokumen"));
                }
            }
        }
        ...
        if (count($data["dokumen-pendukung"]) > 1) {
            ...
            foreach ($data["dokumen-pendukung"] as $dokumen_pendukung) {
                $id_document = $faker->uuid3() . '.' . $dokumen_pendukung->getClientOriginalExtension();
                array_push($list_id_document_pendukung, $id_document);
                // moveFileTemp($dokumen_pendukung, $id_document);
                $dokumen_pendukung->move(public_path("/contract-managements/dokumen-pendukung-contract"), $id_document);
            }
            $contract->dokumen_pendukung = join(",", $list_id_document_pendukung);
        } else {
            $id_document = $faker->uuid3() . '.' . $data["dokumen-pendukung"][0]->getClientOriginalExtension();
            // moveFileTemp($data["dokumen-pendukung"][0], $id_document);
            $data["dokumen-pendukung"][0]->move(public_path("/contract-managements/dokumen-pendukung-contract"), $id_document);
    ...
    public function pendingIssueContractUpload(Request $request, PendingIssue $pendingIssue)
    {
        ...
        if(isset($data['file-document'])){
            ...
            $uploadFinal->save();
            $data['file-document']->move(public_path('contract-managements/dokumen-pending-issue'), $id_document);
        ...
        $pendingIssue->profit_center = $contract->profit_center ?? "-";
    ...
    public function usulanPerubahanDraftContractUpload(Request $request, UsulanPerubahanDraft $usulanPerubahanDraft)
    {
        ...
        $usulanPerubahanDraft->profit_center = $contract->profit_center ?? "-";
    ...
    public function uploadAsuransi(Request $request, ContractAsuransi $asuransi) {
        ...
        if(isset($data['file-document'])){
            ...
            $file->move(public_path('contract-managements/dokumen-asuransi'), $id_document);
        }
        ...
        $asuransi->id_contract = $contract->id_contract;
        $asuransi->profit_center = $contract->profit_center;
    ...
    public function editAsuransi(Request $request, ContractAsuransi $asuransi) {
        ...
        if(isset($data['file-document'])){
            ...
            if(!empty($id_document_asuransi)){
                ...
                File::delete(public_path("contract-managements/dokumen-asuransi/$old_file"));
    ...
    public function uploadJaminan(Request $request, ContractJaminan $jaminan) {
        ...
        if(isset($data['file-document'])){
            ...
            $file->move(public_path("contract-managements/dokumen-jaminan"), $id_document);
        }
        ...
        $jaminan->id_contract = $contract->id_contract;
        $jaminan->profit_center = $contract->profit_center;
    ...
    public function editJaminan(Request $request, ContractJaminan $jaminan) {
        ...
        if(isset($data['file-document'])){
            ...
            if(!empty($id_document_jaminan)){
                ...
                File::delete(public_path("contract-managements/dokumen-jaminan/$old_file"));
                $document->id_document = $id_document;
                $document->nama_document = $nama_file;
                $jaminan->id_document = $id_document;
                $jaminan->nama_document = $nama_file;
    ...
    public function uploadDokumenFinal(Request $request)
    {
        ...
        if (is_array($file)) {
            ...
            foreach ($file as $f) {
                $collectNameFile = [];
                $collectNameFile["id_document"] = date("dmYHis_") . str_replace(' ', '-', $f->getClientOriginalName());
        ...
        // $contract = ContractManagements::find($data["id-contract"]);
        $contract = ContractManagements::where(function ($query) use ($data) {
            $query->where('id_contract', "=", $data['id-contract'])
            ->orWhere('profit_center', '=', $data["id-contract"]);
        })->first();
        ...
        if (empty($contract->id_contract)) {
            ContractManagements::where(function ($query) use ($data) {
                $query->where('id_contract', "=", $data['id-contract'])
                ->orWhere('profit_center', '=', $data["id-contract"]);
            })->update(['id_contract' => Str::uuid()->toString()]);
        }
        ...
        if (in_array($data["kategori"], $filterAddRecord)) {
            ...
            if (is_array($file)) {
                foreach ($file as $i => $f) {
                    ...
                    $f->move(public_path('contract-managements/' . $data['kategori-path']), $collectFile[$i]["id_document"]);
                }
                Alert::success("Success", "Dokumen berhasil ditambahkan");
                return redirect()->back();
            } else {
                ...
                if ($uploadFinal->save()) {
                    ...
                    $file->move(public_path('contract-managements/' . $data['kategori-path']), $id_document);
                    Alert::success("Success", "Dokumen berhasil ditambahkan");
                    return redirect()->back();
                }
                Alert::error("Erorr", "Dokumen gagal ditambahkan");
            ...
            // File::delete(public_path("words/$old_file"));
            File::delete(public_path("contract-managements/" . $data['kategori'] . "/$old_file"));
            $kategori->id_document = $id_document;
            $kategori->nama_document = $nama_file;
            $kategori->save();
            // moveFileTemp($file, explode(".", $id_document)[0]);
            $file->move(public_path('contract-managements/' . $data['kategori-path']), $id_document[0]);
            Alert::success("Success", "Dokumen berhasil ditambahkan");
            return redirect()->back();
        } else{
            // dd("insert empty 2");
    ...
    public function editDokumenFinal(Request $request, $id)
    {
        ...
        if ($kategori->save()) {
            if (!empty($data["file-document"])) {
                // moveFileTemp($file, explode(".", $id_document)[0]);
                $file->move(public_path('contract-managements/' . $data['kategori']), $id_document[0]);
            }
    ...
    {
        $data = $request->all();
        $kategori = ContractUploadFinal::where('id', $id)->first();
        $old_file = $kategori->id_document;
        $path = "contract-managements/" . $data['kategori-path'];
        // File::delete(public_path("words/$old_file"));
    ...
    public function uploadPasalKontraktual(Request $request) {
        ...
        if (empty($contract)) {
            Alert::error("Error", "Contract tidak ditemukan");
            return redirect()->back();
        }
        
        $kontraktual = new PasalKontraktual();
        $kontraktual->id_contract = $contract->id_contract;
        $kontraktual->profit_center = $contract->profit_center;
    ...
    public function uploadDokumenContract(Request $request, $kategori_file)
    {
        try {
            $data = $request->all();
            $maxSize = env('MAX_FILE_SIZE');
            ...
            $spk_intern_no = $contract->ProyekPisNew?->spk_intern_no;
            ...
            if (empty($modelClass)) {
                Alert::error('Error', "Kategori dokumen tidak ditemukan");
                return redirect()->back();
            }


            DB::transaction(function () use ($modelClass, $spk_intern_no, $contract, $data, $kategori_file) {
                foreach ($data['file-document'] as $file) {
                    $namaFile = $file->getClientOriginalName();
                    $idFile = date('dmYHis_') . str_replace(' ', '', $file->getClientOriginalName());

                    $modelClass->spk_intern_no = $spk_intern_no;
                    $modelClass->profit_center = $contract->profit_center;
                    $modelClass->id_document = $idFile;
                    $modelClass->nama_document = $namaFile;
                    $modelClass->status = $data["status"];

                    $modelClass->save();

                    $file->move(public_path('contract-managements/' . $kategori_file), $idFile);
                }
            });

            Alert::success('Success', "Upload Dokumen Berhasi");
            return redirect()->back();
        } catch (\Exception $e) {
    ...
    public function updateDokumenContract(Request $request, $kategori_file, $id)
    {
        try {
            $data = $request->all();
            $maxSize = env('MAX_FILE_SIZE');
            ...
            DB::transaction(function () use ($modelClass, $spk_intern_no, $contract, $data, $kategori_file, $file, $id) {

                $dokumen = $modelClass->find($id);

                $idDocumentOld = $dokumen->id_document;

                $namaFile = $file->getClientOriginalName();
                $idFile = date('dmYHis_') . str_replace(' ', '', $file->getClientOriginalName());

                $dokumen->spk_intern_no = $spk_intern_no;
                $dokumen->profit_center = $contract->profit_center;
                $dokumen->id_document = $idFile;
                $dokumen->nama_document = $namaFile;
                $dokumen->status = $data["status"];

                $dokumen->save();

                if (File::delete(public_path('contract-managements/' . $kategori_file . '/' . $idDocumentOld))) {
                    $file->move(public_path('contract-managements/' . $kategori_file), $idFile);
                } else {
                    Alert::error('Error', "Dokumen gagal diupdate. Hubungi Admin!");
                    return redirect()->back();
                }
            });

            Alert::success('Success', "Update Dokumen Berhasil");
            return redirect()->back();
        } catch (\Exception $e) {
            Alert::error('Error', $e->getMessage());
            return redirect()->back();
    ...
    public function deleteDokumenContract(Request $request, $kategori_file, $id)
    {
        $data = $request->all();
        ...
        try {
            ...
            if (empty($contract)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Kontrak tidak ditemukan. Silahkan hubungi Admin!'
                ]);
            }
            ...
            DB::transaction(function () use ($modelClass, $kategori_file, $id) {
                $dokumen = $modelClass->find($id);

                if (empty($dokumen)) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Dokumen tidak ditemukan. Silahkan hubungi Admin!'
                    ]);
                }

                $idDokumenOld = $dokumen->id_document;

                if ($dokumen->delete()) {
                    File::delete(public_path('contract-managements/' . $kategori_file . '/' . $idDokumenOld));
                }
            });

...
@section('title', 'Verifikasi Internal Partner')
...
@section('content')
                    ...
                    <div class="toolbar" id="kt_toolbar">
                        ...
                        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
                            ...
                            <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                                ...
                                <!--begin::Title-->
                                <h1 class="d-flex align-items-center fs-3 my-1">Approval Verifikasi Internal Penentuan KSO / Non KSO
                                </h1>
                                <!--end::Title-->
                            </div>
                            <!--begin::Page title-->
                                            ...
                                            @foreach ($dataProyekProsesVerifikasi as $item)
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        ...
                                                        @canany(['access-menu-approve'], 'VPPS')
                                        ...
                                        <tbody>
                                            ...
                                            @foreach ($dataProyekFinishVerifikasi as $item)
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        {{-- @canany(['super-admin', 'approver-crm', 'risk-crm', 'admin-crm', 'user-crm']) --}}
                                                        @canany(['access-menu-read', 'access-menu-lock', 'access-menu-approve'], 'VPPS')
                                                            @if (!is_null($item->is_persetujuan_approved) && $item->is_persetujuan_approved)
                                                                <a href="{{ asset('file-nota-rekomendasi-2\\file-verifikasi-internal-partner\\') . $item->nama_dokumen }}" class="btn btn-sm btn-primary text-white" target="_blank">Download</a>
                                                            @endif
                                                        @endcanany
                                                    </td>
                                                </tr>
    ...
    @foreach ($dataProyekProsesVerifikasi as $proyek)
        <form action="/verifikasi-internal-partner/pengajuan/{{ $proyek->id }}" method="post" onsubmit="addLoading(this)">
            @csrf
            <div class="modal fade" id="kt_modal_pengajuan_verifikasi_{{ $proyek->kode_proyek }}" tabindex="-1"
                ...
                <div class="modal-dialog modal-dialog-centered modal-xl">
                    <div class="modal-content">
                        <input type="hidden" name="modal-name" value="kt_modal_pengajuan_verifikasi_{{ $proyek->kode_proyek }}">
                        ...
                        <div class="modal-body">
                            ...
                            <br>
                            <hr>
                            <h2>Form Verifikasi Penentuan KSO</h2>
                            <div class="text-center">
                                <iframe src="{{ asset("file-nota-rekomendasi-2\\file-verifikasi-internal-partner\\") . $proyek->nama_dokumen }}" width="800px" height="auto"></iframe>
                            </div>
                        </div>
                        ...
                        <div class="modal-footer">
                            <button class="btn btn-sm btn-danger" onclick="showModalRevisi('{{ $proyek->kode_proyek }}', 'pengajuan', '{{ $proyek->id }}')" type="button">Ajukan Revisi</button>
        ...
        <form action="/verifikasi-internal-partner/rekomendasi/{{ $proyek->id }}" method="post" onsubmit="addLoading(this)">
            @csrf
            <div class="modal fade" id="kt_modal_rekomendasi_verifikasi_{{ $proyek->kode_proyek }}" tabindex="-1"
                ...
                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl">
                    <div class="modal-content">
                        ...
                        @else
                            <div class="modal-footer">
                                <button class="btn btn-sm btn-danger" onclick="showModalRevisi('{{ $proyek->kode_proyek }}', 'rekomendasi', '{{ $proyek->id }}')" type="button">Ajukan Revisi</button>
        ...
        <form action="/verifikasi-internal-partner/persetujuan/{{ $proyek->id }}" method="post" onsubmit="addLoading(this)">
            @csrf
            <div class="modal fade" id="kt_modal_persetujuan_verifikasi_{{ $proyek->kode_proyek }}" tabindex="-1"
                ...
                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-xl">
                    <div class="modal-content">
                        ...
                        @else
                        <div class="modal-footer">
                            <button class="btn btn-sm btn-danger" onclick="showModalRevisi('{{ $proyek->kode_proyek }}', 'persetujuan', '{{ $proyek->id }}')" type="button">Ajukan Revisi</button>
                            <input type="submit" name="is_approved" value="Setujui" class="btn btn-sm btn-success">
                        </div>
                        @endif
                    </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        @php
                            ...
                            if ($pengajuan->isNotEmpty() || $persetujuan->isNotEmpty() || $rekomendasi->isNotEmpty()) {
                                $data_approved_merged = collect()->mergeRecursive(['Pengajuan' => $pengajuan->flatten(), 'Rekomendasi' => $rekomendasi->flatten(), 'Persetujuan' => $persetujuan->flatten()]);
                            }
                        @endphp
                        {{-- Begin :: History --}}
                        <div class="row">
...
@section('js-script')
    ...
    <script>
        ...
        async function showModalAction(id_modal, kodeProyek) {
            ...
            if (dataProyek.verifikasi_internal_partner != null) {
                if (dataProyek.verifikasi_internal_partner.persetujuan_approved != null) {
                    dataPersetujuanApproval = JSON.parse(dataProyek.verifikasi_internal_partner.persetujuan_approved);
                }

                if (dataProyek.verifikasi_internal_partner.rekoemdasi_approved != null) {
                    dataRekomendasiApproval = JSON.parse(dataProyek.verifikasi_internal_partner.rekoemdasi_approved);
                }

                if (dataProyek.verifikasi_internal_partner.pengajuan_approved != null) {
                    dataPengajuanApproval = JSON.parse(dataProyek.verifikasi_internal_partner.pengajuan_approved);
                }

                if (dataProyek.verifikasi_internal_partner.request_pengajuan != null) {
                    dataRequestApproval = JSON.parse(dataProyek.verifikasi_internal_partner.request_pengajuan);
                }
            }
            ...
            if (id_modal == "kt_modal_pengajuan") {
                ...
                htmlFooter = `
                <button class="btn btn-sm btn-danger" onclick="showModalRevisi()" type="button">Ajukan Revisi</button>
        ...
        function showModalRevisi(kode_proyek, kategori, id) {
            ...
            const form = modalId.parentElement.setAttribute('action', `/verifikasi-internal-partner/${kategori}/${id}`)
    ...
    <script>
        $(document).ready(function() {
            const tes = window.location.search;

...
@section('content')
                    ...
                    <div class="toolbar" id="kt_toolbar">
                        ...
                        <div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
                            ...
                            <div data-kt-swapper="true" data-kt-swapper-mode="prepend"
                                ...
                                <!--begin::Title-->
                                <h1 class="d-flex align-items-center fs-3 my-1">Role Managements
                                </h1>
                                <!--end::Title-->
                            </div>
                            <!--end::Page title-->
                            ...
                            @if (auth()->user()->check_administrator || str_contains(auth()->user()->name, "(PIC)"))
                                ...
                                <div class="d-flex align-items-center py-1">
                                    ...
                                    <!--begin::Button-->
                                    <a  href="#" data-bs-target="#kt_modal_view_tambah_role" data-bs-toggle="modal" class="btn btn-sm btn-primary py-3"
                            ...
                            <table class="table align-middle table-bordered border-dark fs-6 gy-2" id="example">
                                ...
                                <thead>
                                    ...
                                    <tr class="text-start text-gray-400 fw-bolder fs-7 text-uppercase gs-0 bg-primary">
                                        ...
                                        <th class="min-w-auto text-white">Aplikasi</th>
                                        <th class="min-w-auto text-white">Role</th>
                                ...
                                <tbody class="fw-bold text-gray-600">
                                    ...
                                    @endphp --}}
                                        <tr>
                                            ...
                                            <td class="text-center align-middle">{{ $item->aplikasi }}</td>
                                            {{-- <td class="text-center">{{$start_bulan}}</td> --}}
                                            {{-- <td class="text-center">{{$finish_bulan}}</td> --}}
                                            <td class="text-center align-middle">{{ $item->role }}</td>
                                            <td class="text-center align-middle">{{ $item->is_active == true ? "Active" : "Not Active" }}</td>
                                            <td class="text-center align-middle">
                                                <div class="d-flex justify-content-center gap-2">
                                                    <a href="#" data-bs-target="#kt_modal_update_role_{{$item->id }}" data-bs-toggle="modal" class="btn btn-sm btn-primary text-white" style="background-color: #008CB4;">Edit</a>
                                                        <input type="hidden" name="id-role-management" value="{{$item->id }}">
    ...
    <div class="modal fade" id="kt_modal_view_tambah_role">
        <!--begin::Modal dialog-->
        <div class="modal-dialog modal-dialog-centered mw-600px">
            ...
            <div class="modal-content">
                ...
                <form action="/role-management/save" method="POST">
                    @csrf
                    ...
                    <div class="modal-body py-lg-6 px-lg-6">
                        ...
                        <div class="row fv-row">
                            ...
                            <div class="row mb-7">
                                 <!--begin::Label-->
                                 <label class="fs-6 fw-bold form-label mt-3">
                                    <span class="required">Aplikasi</span>
                                </label>
                                <!--end::Label-->
                                <div class="d-flex flex-row gap-2">
                                    <!--begin::Input-->
                                    <select id="aplikasi" name="aplikasi"
                                        class="form-select form-select-solid select2-hidden-accessible"
                                        data-control="select2" data-hide-search="true" data-placeholder="Pilh Aplikasi"
                                        data-select2-id="select2-aplikasi" tabindex="-1" aria-hidden="true" onchange="setItem(this)">
                                        <option value=""></option>
                            ...
                            <div class="row mb-7">
                               ...
                               <div class="d-flex flex-row gap-2">
                                   ...
                                   <select id="role-select" name="role"
                                       class="form-select form-select-solid select2-hidden-accessible"
                                       data-control="select2" data-hide-search="true" data-placeholder="Pilh Role"
                                       data-select2-id="role-select" tabindex="-1" aria-hidden="true">
                                       <option value="" selected></option>
                                   </select>
                                   <!--end::Input-->
                               </div>
        ...
        <div class="modal fade" id="kt_modal_update_role_{{ $item->id }}">
            ...
            <div class="modal-dialog modal-dialog-centered mw-600px">
                ...
                <div class="modal-content">
                    ...
                    <form action="/role-management/update/{{ $item->id }}" method="POST">
                        @csrf
                        <input type="hidden" name="modal" value="kt_modal_create_otomasi_approval">
                        <!--begin::Modal body-->
                        <div class="modal-body py-lg-6 px-lg-6">
                            ...
                            <div class="row fv-row">
                                ...
                                <div class="row mb-7">
                                    ...
                                    <div class="d-flex flex-row gap-2">
                                        <!--begin::Input-->
                                        <select id="nama_pegawai_{{ $item->id }}" name="nama_pegawai"
                                            class="form-select form-select-solid select2-hidden-accessible"
                                            data-control="select2" data-hide-search="false" data-placeholder="Pilh Pegawai"
                                            data-select2-id="select2-nama-pegawai-{{ $item->id }}" tabindex="-1" aria-hidden="true" readonly>
                                            <option value="{{ $item->nama_pegawai }}">{{ $item->nama_pegawai }} - {{ $item->Pegawai->nama_pegawai }}</option>
                                            {{-- @foreach ($all_pegawai as $pegawai)
                                                <option value="{{ $pegawai->nip }}">{{ $pegawai->nip }} - {{ $pegawai->nama_pegawai }}</option>
                                            @endforeach --}}
                                        </select>
                                ...
                                <div class="row mb-7">
                                   ...
                                   <div class="d-flex flex-row gap-2">
                                       <!--begin::Input-->
                                       <select id="aplikasi-{{ $item->id }}" name="aplikasi"
                                           class="form-select form-select-solid select2-hidden-accessible"
                                           data-control="select2" data-hide-search="true" data-placeholder="Pilh Aplikasi"
                                           data-select2-id="select2-aplikasi-{{ $item->id }}" tabindex="-1" aria-hidden="true" onchange="setItem(this, '{{ $item->id }}')">
                                           <option value=""></option>
                                           <option value="CRM" {{ $item->aplikasi == "CRM" ? 'selected' : '' }}>CRM</option>
                                           <option value="CCM" {{ $item->aplikasi == "CCM" ? 'selected' : '' }}>CCM</option>
                                           <option value="CSI" {{ $item->aplikasi == "CSI" ? 'selected' : '' }}>CSI</option>
                                       </select>
                                       <!--end::Input-->
                                   </div>
                               </div>
                                ...
                                <div class="row mb-7">
                                   ...
                                   <div class="d-flex flex-row gap-2">
                                       <!--begin::Input-->
                                       <select id="role-select-{{ $item->id }}" name="role"
                                           class="form-select form-select-solid select2-hidden-accessible"
                                           data-control="select2" data-hide-search="true" data-placeholder="Pilh Role"
                                           data-select2-id="role-select-{{ $item->id }}" tabindex="-1" aria-hidden="true">
                                           <option value="" selected></option>
                                           @if ($item->aplikasi == 'CRM')
                                                <option value="Admin" {{ $item->role == "Admin" ? 'selected' : '' }}>Admin</option>
                                                <option value="User" {{ $item->role == "User" ? 'selected' : '' }}>User</option>
                                                <option value="Approver" {{ $item->role == "Approver" ? 'selected' : '' }}>Approver</option>
                                                <option value="Risk" {{ $item->role == "Risk" ? 'selected' : '' }}>Risk</option>
                                            @elseif ($item->aplikasi == 'CCM')
                                                <option value="Admin" {{ $item->role == "Admin" ? 'selected' : '' }}>Admin</option>
                                                <option value="User" {{ $item->role == "User" ? 'selected' : '' }}>User</option>
                                                <option value="Approver" {{ $item->role == "Approver" ? 'selected' : '' }}>Approver</option>
                                            @else
...
<script>
    $(document).ready(function() {
        $("#nama_pegawai").select2({
            dropdownParent: $("#kt_modal_view_tambah_role")
        });
    });
        ...
        Swal.fire({
            ...
            if(result.isConfirmed){
                try {
                    ...
                    const req = await fetch(`{{ url('/role-management/delete') }}/${id}`, {
                        method: 'POST',
                        header: {
    ...
    function setItem(elt, id=null, text=null) {
        if(elt.value == "CRM") {
            ...
            if (id != null) {
                $(`#role-select-${id}`).empty() 
                $(`#role-select-${id}`)
                $(`#role-select-${id}`).select2({
